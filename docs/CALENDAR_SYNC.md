...
5. Add `https://www.krain.studio/api/calendar-sync/callback/google` as its authorised redirect URL.
6. Enable Google Calendar API and use the `calendar` scope.
7. Put the IDs and secrets in the private Contabo environment file.
8. Open `https://www.krain.studio/calendar-sync` and connect Microsoft first, then Google.
